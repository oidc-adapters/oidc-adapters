...
describe('key-provider.ts', function () {
  ...
  it('should not retrieve PEM key from denied issuer (regex)', async () => {
    const body = new URLSearchParams()
    body.append('client_id', 'admin-cli')
    body.append('username', 'admin')
    body.append('password', 'admin')
    ...
    const keyProvider = new KeyProvider({
      allowedIssuers: ['http://localhost:8109/realms/master'],
      deniedIssuers: [/http:\/\/localhost:8109\/.*/]
    })
    await expect(keyProvider.getPublicKey(accessToken)).rejects.toThrow('Token issuer "http://localhost:8109/realms/master" is denied')
  })
})
