...
import { Controller, Delete, Get, Global, Module, Request, UseGuards } from '@nestjs/common'
...
describe('PermissionBasedAccessControlModule with decorators (forRootAsync useFactory)', () => {
  let app: INestApplication
  ...
  beforeEach(async () => {
    ...
    const testingModule = await Test.createTestingModule({
      imports: [
        OidcPassportModule.forRootAsync({ useFactory: () => ({ options: { allowedIssuers: ['http://localhost:8109/realms/keycloak-test'] } }) }),
        PermissionBasedAccessControlModule.forRootAsync({
          imports: [ConfigModule],
          inject: ['providerType'],
          useFactory: (providerType: 'user') => ({
